...
describe('application reducer', () => {
  ...
  describe('updateBlockNumber', () => {
    it('updates block number', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BSCMAINNET, blockNumber: 4 }))
      expect(store.getState().blockNumber[ChainId.BSCMAINNET]).toEqual(4)
    })
    it('no op if late', () => {
      store.dispatch(updateBlockNumber({ chainId: ChainId.BSCMAINNET, blockNumber: 2 }))
      expect(store.getState().blockNumber[ChainId.BSCMAINNET]).toEqual(3)
    })
